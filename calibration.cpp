...
static void calcChessboardCorners(Size boardSize, float squareSize, vector<Point3f> &corners, Pattern patternType = CHESSBOARD)
{
    ...
    case ASYMMETRIC_CIRCLES_GRID: //不对称圆点标定板
        for (int i = 0; i < boardSize.height; i++)
            for (int j = 0; j < boardSize.width; j++)
...
//标定计算
static bool runCalibration(vector<vector<Point2f>> imagePoints, //平面点
                           Size imageSize, Size boardSize, Pattern patternType,
                           float squareSize, float aspectRatio, //fx/fy
                           ...
                           double &totalAvgErr)
{
    ...
    distCoeffs = Mat::zeros(8, 1, CV_64F); //Output vector of distortion coefficients

    vector<vector<Point3f>> objectPoints(1);                                    //物方点，即棋盘格上的点
    calcChessboardCorners(boardSize, squareSize, objectPoints[0], patternType); //计算棋盘格角点
...
int main(int argc, char **argv)
{
    ...
    //                              "{help ||}{w|4|}{h|7|}{pt|chessboard|}{n|10|}{d|1000|}{s|0.02423|}{o|out_camera_data.yml|}"
    //                              "{op||}{oe||}{zt||}{a||}{p||}{v||}{V||}{su||}"
    //                              "{oo||}{ws|11|}{dt||}"
    //                              "{@input_data|../imagelist2.xml|}");
    cv::CommandLineParser parser(argc, argv,
                                 "{help ||}{w|7|}{h|9|}{pt|chessboard|}{n|10|}{d|1000|}{s|0.02|}{o|out_camera_data3.yml|}"
                                 "{op||}{oe||}{zt||}{a||}{p||}{v||}{V||}{su||}"
                                 "{oo||}{ws|11|}{dt||}"
                                 "{@input_data|../imagelist3.xml|}");
    if (parser.has("help"))
    {
        help();
        return 0;
